...
folder.name <- function(name){
  ...
  filename <- str_c(name," ",hour,minute," ",month," ",day," ",year)
  
  return(filename)
}

make.foodgroups <- function(){
  
  if ("Food" %in% names(data)){
    
    foodgroups  <- unique(data$Food[data$Food$Selection == TRUE,]$`Food Group`)
    people      <- read.individuals(filename)$Individual
    
    x           <- data.frame(matrix(ncol = length(foodgroups)+1, nrow = length(people)))
    colnames(x)[2:ncol(x)] <- foodgroups
    colnames(x)[1] <- "Individuals"
    x$Individuals  <- people
  }
  return(x)
}

make.cover <- function(){
  msg1 <- str_c("Run minimum sample size set to ",minsamples,".")
  msg2 <- "Please Fill in the Blanks on the 'Food Groups' sheet."
  msg3 <- "Column 'Selection' added. Add Food-to-Group weights for rows marked TRUE."
  msg4 <- "Run Function 2 after assigning food weights."
  
  x <- data.frame(Cover = c(msg1,msg2,msg3,msg4))
  
  return(x)
}

create.project.file<- function(){
  ## names 
  title   <- substr(filename,0,as.numeric(regexpr("\\.[^\\.]*$",filename))-1)
  project <- folder.name(title)
  
  data             <-append(data,list(Cover,`Food Groups`),0)
  names(data)[1:2] <- c("Cover","Food Groups")
  F2GW             <- c(rep("input required", sum(data$Food$Selection))
                        ,rep("Insufficient Data", nrow(data$Food)-sum(data$Food$Selection)))
  data$Food        <- cbind(`Food to Group Weight` = F2GW,data$Food)
  ...
  dir.create(project)
  setwd(str_c("./",project))
  export(data,str_c(title,".xlsx"))
  setwd("../")
  setwd("../")
  setwd("./R/")
  
  ## messaging ## 
  line1 <<- str_c("Project Folder '", title, "' created.")
  cat(" \n",
      line1, "\n","\n",
      "Enter all Food Group weights (Sheet 2) and" ,"\n",
      "Food-to-Group weights (Column 1, 'Food' sheet)" ,"\n",
      "before continuing to the exposure model." ,"\n","\n",
      "For all media, Data marked 'FALSE' in 'Selection' column lack sufficient ","\n",
      "summary statisticss or sample size to estimate GM and GSD and are not considered in this model.","\n")
  return()
}